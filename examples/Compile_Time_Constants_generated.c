...
struct String Constexpr__Lookup__RGBMap(struct String *p_string);
...
struct String Constexpr__Lookup__RGBMap(struct String *p_string) {
  if (String__eq__(p_string, "Red")) {
    struct String result;
    String__init__from_charptr(&result, "225,0,0", 7);
    return result;
  } else if (String__eq__(p_string, "Green")) {
    struct String result;
    String__init__from_charptr(&result, "0,255,0", 7);
    return result;
  }
  struct String result;
  String__init__from_charptr(&result, "255,255,255", 11);
  return result;
}
...
int main() {
  ...
  struct String red;
  String__init__from_charptr(&red, "225,0,0", 7);
  printf("Red Color RGB: %s \n", Stringc_str(&red));

  struct String green;
  String__init__from_charptr(&green, "0,255,0", 7);
  printf("Green Color RGB: %s \n", Stringc_str(&green));

  struct String default_color;
  String__init__from_charptr(&default_color, "255,255,255", 11);
  printf("Default Color RGB: %s \n", Stringc_str(&default_color));

  struct String g_key;
  String__init__OVDstrint(&g_key, "Green", 5);
  struct String contexpr_at_runtime_color = Constexpr__Lookup__RGBMap(&g_key);
  printf("Green Color RGB fetched from constexpr map at runtime: %s \n",
         Stringc_str(&contexpr_at_runtime_color));

  String__del__(&contexpr_at_runtime_color);
  String__del__(&g_key);
  String__del__(&default_color);
  String__del__(&green);
  String__del__(&red);
