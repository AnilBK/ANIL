...
void HTTPServeradd_validation_rule(struct HTTPServer *this, char *route_name,
                                   char *method, char *required_content_type);
...
void HTTPServerregister_route(struct HTTPServer *this, char *method,
  ...
  route.has_validation_rules =
      false; // Default to false, can be set later if needed
...
int main() {
  ...
  HTTPServeradd_validation_rule(&server, "/save_todos", "OPTIONS", "");
  HTTPServeradd_validation_rule(&server, "/save_todos", "POST",
                                "application/json");
